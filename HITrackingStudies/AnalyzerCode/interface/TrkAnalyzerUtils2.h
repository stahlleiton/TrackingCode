#ifndef TrkAnalyzerUtils2_h_
#define TrkAnalyzerUtils2_h_
...
float rndSF2(float value, int nSignificantDigits) 
{
  if(value==0) return 0; 
...
inline float rndDP2(float value, int nPlaces)
{
  return float(int(value*pow(10,nPlaces))/pow(10,nPlaces));
}

//----------------------------------------------------------------------------------------------------
const TrackingParticle* doRecoToTpMatch2(reco::RecoToSimCollection recSimColl, const reco::TrackRef &in){
...
vector<int> matchTpToGen2(const edm::Event& iEvent, const TrackingParticle* tparticle, edm::EDGetTokenT<reco::GenParticleCollection> genCollection){
