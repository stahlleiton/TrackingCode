  ...
  for (int i = 0; i < vec_ori.size(); i++)
	...
	{     
	  if(hptracks)
	    ...
	    {
	      if(vec_trk_algo[i]==algoToPlot) v.push_back(vec_ori[i]);
	    }
	}
      else
	{
  ...
  for (int i = 0; i < vec_num.size(); i++)
	...
	{     
	  if(hptracks)
	    ...
	    {
	      if(vec_trk_algo[i]==algoToPlot) v.push_back(vec_num[i]/vec_den[i]);
	    }
	}
      else
	{
	  if(runOnPixelTrks)  // pixel tracks
		...
		{	 
	      ...
	      else v.push_back(vec_num[i]/vec_den[i]);
	    }
	}
    }
  return v;
...
std::vector<float> func_chi2_select_trks( ROOT::VecOps::RVec<Float_t> vec_chi2, ROOT::VecOps::RVec<Float_t> vec_ndf,ROOT::VecOps::RVec<Float_t> vec_nlayers, ROOT::VecOps::RVec<Bool_t> vec_trk_fake, ROOT::VecOps::RVec<Bool_t> vec_trk_hp, ROOT::VecOps::RVec<Int_t> vec_trk_algo, ROOT::VecOps::RVec<Float_t> vec_trk_pt)
{
  ...
  for (int i = 0; i < vec_chi2.size(); i++)
	...
	{     
	  if(hptracks)
	    {
	      if ( vec_trk_hp[i] && vec_trk_algo[i]==algoToPlot ) v.push_back(vec_chi2[i]/vec_nlayers[i]);
	    }
	  else 
	    {
	      if(vec_trk_algo[i]==algoToPlot) v.push_back(vec_chi2[i]/vec_nlayers[i]);
	    }
	}
      else
	{
	  if(runOnPixelTrks) // pixel tracks
	    ...
	    {	 
	      if(hptracks)
		{
		  if ( vec_trk_hp[i] ) v.push_back(vec_chi2[i]/vec_nlayers[i]);		 
		}
	      else v.push_back(vec_chi2[i]/vec_nlayers[i]);
	    }
	}
    }
  return v;
...
void macro_control_plots_OneSample_2018Data()
{
  ...
  TString fig_name[N_variables]={"hist_Vz_NoSel_Cent010_Run2Data.pdf", "hist_HiBin_NoSel_Cent010_Run2Data.pdf", "hist_pt_allTrks_NoSel_Cent010_Run2Data.pdf","hist_eta_allTrks_NoSel_Cent010_Run2Data.pdf","hist_phi_allTrks_NoSel_Cent010_Run2Data.pdf","hist_nhits_allTrks_NoSel_Cent010_Run2Data.pdf","hist_dZsig_allTrks_NoSel_Cent010_Run2Data.pdf","hist_dXYsig_allTrks_NoSel_Cent010_Run2Data.pdf","hist_pTres_allTrks_NoSel_Cent010_Run2Data.pdf","hist_chi2_allTrks_NoSel_Cent010_Run2Data.pdf","hist_algo_allTrks_NoSel_Cent010_Run2Data.pdf","hist_nhits_allTrks_NimusOneCuts_Cent010_Run2Data.pdf","hist_dZsig_allTrks_NimusOneCuts_Cent010_Run2Data.pdf","hist_dXYsig_allTrks_NimusOneCuts_Cent010_Run2Data.pdf","hist_pTres_allTrks_NimusOneCuts_Cent010_Run2Data.pdf","hist_chi2_allTrks_NimusOneCuts_Cent010_Run2Data.pdf"}; //Name of the figure file with the histograms saved
  ...
  TFile* fout = new TFile("ControlPlotsOneSample/Run2Data_Cent010_CentTable.root", "RECREATE");
  fout->cd();
